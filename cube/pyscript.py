...
try:
    with open("C:/Screensavers/cube/config.yaml","r") as f:
        loadyaml = yaml.safe_load(f)
        readsizex,readsizey,readdimmed = loadyaml["sizex"],loadyaml["sizey"],loadyaml["dimmed"]
    ...
    for e in pygame.event.get():
        if e.type in (pygame.KEYDOWN,pygame.MOUSEMOTION) and init:
            ...
            try:
                ...
                screen.fill((0,0,0))
                screen.blit(toprint,(960-(nsizex/2),540-(nsizey/2)))
                pygame.display.update(pygame.Rect(960-(sizex/2),540-(sizey/2),sizex,sizey))
                sizex,sizey = nsizex,nsizey
            with open("C:/Screensavers/cube/config.yaml","w+") as f:
                yaml.dump({"sizex":sizex,"sizey":sizey,"dimmed":wasdimmed},f)
    init = True
    frame += 1
    if frame == 60:
        frame = 0
