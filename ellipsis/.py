...
try:
    with open("C:/Screensavers/ellipsis/.config","r") as f:
        readColour,readRainbow,readDimmed = f.read().split(",")
    ...
    for e in pygame.event.get():
        if e.type in (pygame.KEYDOWN,pygame.MOUSEMOTION) and init:
            ...
            if e.button == 2:
                ...
                pygame.quit()
                sys.exit()
            with open("C:/Screensavers/ellipsis/.config","w+") as f:
                f.write(str(dotColour[dotToAppend])+","+str(int(rainbow))+","+str(setDimmed))
    fps.tick(60)
    frame += 1
    dimFrames += 1
    if dimFrames == 108000:
